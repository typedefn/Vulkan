...
public:
	...
	~VulkanExample()
	{
		...
		vkDestroyBuffer(device, verticies.buffer, nullptr);
		vkFreeMemory(device, verticies.memory, nullptr);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			vkCmdBindVertexBuffers(drawCmdBuffers[i], 0, 1, &verticies.buffer, offsets);
	...
	void prepareVertices(bool useStagingBuffers)
	{
		...
		if (useStagingBuffers)
		{
			...
			VK_CHECK_RESULT(vkCreateBuffer(device, &vertexBufferInfo, nullptr, &verticies.buffer));
			vkGetBufferMemoryRequirements(device, verticies.buffer, &memReqs);
			memAlloc.allocationSize = memReqs.size;
			memAlloc.memoryTypeIndex = getMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			VK_CHECK_RESULT(vkAllocateMemory(device, &memAlloc, nullptr, &verticies.memory));
			VK_CHECK_RESULT(vkBindBufferMemory(device, verticies.buffer, verticies.memory, 0));
			...
			vkCmdCopyBuffer(copyCmd, stagingBuffers.vertices.buffer, verticies.buffer, 1, &copyRegion);
			// Index buffer
		...
		else
		{
			...
			memAlloc.memoryTypeIndex = getMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
			VK_CHECK_RESULT(vkAllocateMemory(device, &memAlloc, nullptr, &verticies.memory));
			VK_CHECK_RESULT(vkMapMemory(device, verticies.memory, 0, memAlloc.allocationSize, 0, &data));
			memcpy(data, vertexBuffer.data(), vertexBufferSize);
			vkUnmapMemory(device, verticies.memory);
			VK_CHECK_RESULT(vkBindBufferMemory(device, verticies.buffer, verticies.memory, 0));
